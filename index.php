...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
    ...
    $sql = "SELECT p.id, p.data, e_local.nom AS equip_local, e_visitant.nom AS equip_visitant, p.gols_local, p.gols_visitant, p.jugat, l.nom AS liga
            FROM partits p
    ...
    $stmt->bindValue(':liga', $lligaSeleccionada, PDO::PARAM_STR);
} else {
    $sql = "SELECT p.id, p.data, e_local.nom AS equip_local, e_visitant.nom AS equip_visitant, p.gols_local, p.gols_visitant, p.jugat, l.nom AS liga
            FROM partits p
            JOIN equips e_local ON p.equip_local_id = e_local.id
...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
    ...
    $totalPartitsStmt->bindValue(':liga', $lligaSeleccionada, PDO::PARAM_STR);
} else {
    $totalPartitsStmt = $conn->prepare("SELECT COUNT(*) 
                                        FROM partits p
                                        JOIN lligues l ON p.liga_id = l.id
